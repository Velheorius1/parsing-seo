...
981 млрд сум и НИ ОДНОЙ записи о том, что после них произошло. Колонка
`winner` заполнена у 2963 строк — и ни одна из них не алертилась: их пишет
results_tracker из CivilContracts, а это отбор по ВМК-69 (не прямые договоры,
как считалось до 24.09: у лота есть срок и адрес оферты), другое пространство
id (`2612 05 <civil_id>` против `2611 0012 <lot_id>` у лотов). То есть
