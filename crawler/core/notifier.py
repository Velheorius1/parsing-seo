...
# БАННЕРЫ И ЧЕКОВАЯ ЛЕНТА — НЕ НАШЕ (25.09, решение Данияра). Чековая лента
# (Cooperation, ТН ВЭД 4811) шла с 90: 144 алерта за всё время, 13 за 22–25.09.
# Слово «чеков» в словаре остаётся — оно же ловит чековые книжки (бланки, 90–95),
# поэтому ленту режет гейт. Баннеры в «НЕ НАШЕ» были только «наружные на
# фасадах», а «печать на ткани» в «МЫ ДЕЛАЕМ» и «широкоформат ТОЛЬКО для
# наружной» модель читала как «печать баннера = типография».
# Замер (flash-0731): 378 баннер/лента-лотов, которых AI не видел, ×3 — верных
# 908 → 1072/1134; контрольные 38 лотов ×4 без мусорных ответов — 112 → 147/152,
# все 19 «наших» соседей держатся (чековые и кассовые книжки, стикеры, стенды,
# плакаты на стекле, Roll Up, «баннер + стенды/бланки/плакаты»).
# Отвергнуты: «Баннеры — любые…» с отсылкой к мульти-лоту — роняла стикеры,
# кассовые книги и стенды (контроль 72 → 51/87); короткое «Баннеры (наружные и
# в помещении, печать и установка)» без причины и примеров — «Reklama Banneri»
# и «Bannerlarni … chop etish» уходили с 0 на 90. Остаток: «Баннер чоп этиш ва
# жалюзи урнатиш» проходит, как и со старым промптом.
#
# ГОТОВЫЕ КНИГИ ДЛЯ БИБЛИОТЕК — НЕ НАШЕ (25.09, решение Данияра). Строка
# «Закупка готовых книг» в «НЕ НАШЕ» была, но мульти-лотовая оговорка ниже
# называла «книги/издания» нашими без условий — и модель ставила 90
# «Бадиий адабиёт», «Дарслик», «kitoblar olish» для колоний: за месяц 114
# алертов по книгам, заметная доля — покупки тиража. Замер (flash-0731, ×3):
# покупка книг в отказ 165/267 → 267/267 в наборе, 258/417 → 417/417 вне
# набора; печать на заказ («chop etish», издательство, типография, книги
# учёта) держится вся. Отвергнуты: формулировка «Закупка ГОТОВЫХ…» —
# растекалась на любой готовый товар (бирка 90→30, плакаты на стекле 90→0);
# без правки оговорки 12 «Дарслик» проходили; оговорка без «книги/издания»
# роняла лот с флагами 0/3. Цена выбранного: «Реклама банери» без подробностей
# (ТН ВЭД 4911) 90 → 0 — по пункту о наружных баннерах это и так не наше.
#
# ОФИСНАЯ БУМАГА — НЕ НАШЕ (24.09, решение Данияра). Правила про бумагу в
# промпте не было, и модель решала сама: «бумага = полиграфия». Решение плавало
# на одном и том же тексте — «Когоз» 30 и 85, «Ок когоз» 20 и 85; прод 24.09
# отправил пушами Kseroks 360 млн и Maxsus 199,5 млн.
# Замер правила (flash-0731, живые строки, каждый лот ×4): 149/152 верных —
# 17 бумажных лотов в отказ, 21 контроль (бланки, формуляры, книги учёта,
# «варақаси», конверты, пакеты, наклейки) держится. Остаток — одиночные
# всплески модели («Когоз» 100 один раз из четырёх), не систематика: вариант
# со словом «офсетная» дал 150/152 и перенёс всплеск на Maxsus — отвергнут.
# 40 случайных принятых лотов — 31 против 30/31 у старого промпта.
# Цена: перечень «перфофайл + конверт + скотч» ушёл с 75 на 30.
# Оговорка про категорию «Бумага и изделия из бумаги» нужна: без неё
# предквалификации с голым названием категории падали с 70 до 30.
...
НЕ НАШЕ:
- Баннеры — наружные и в помещении: их печать (это широкоформат, не наше
  оборудование), установка, люверсы, баннерная ткань; билборды, растяжки на
  фасадах, световые короба, монтаж наружной рекламы:
    «Реклама банери», «Рекламный баннер», «Bannerlarni chop etish» = НЕ наше
  (НО информационные таблички, выставочные стенды, указатели — это НАШЕ)
- Папки, скоросшиватели, канцелярские органайзеры
- Офисная бумага как товар: бумага для оргтехники и ксерокса, писчая или цветная
  бумага листами в пачках — закупка расходника со склада, а не заказ на
  изготовление. Формат и плотность дела не меняют:
    «Бумага для офисной техники белая, А4 80 г/м²»  = НЕ наше
    «бумага А4/А3 плотностью 160–200 г в пачках»    = НЕ наше
  НО бланки, формуляры, журналы и книги учёта, блокноты, конверты, бумажные
  пакеты, наклейки — печатная продукция, НАШЕ, даже если в названии есть
  «бумага», «А4» или «qog'oz». Название категории «Бумага и изделия из бумаги»
  без перечня позиций — само по себе ещё не офисная бумага
- Чековая лента (кассовая, термолента, «чек тасмаси»), в том числе «чековая
  лента печатанная» — расходник для касс, как офисная бумага.
  НО чековые книжки и термоэтикетки — НАШЕ
- Широкоформатная печать для баннеров и наружной рекламы (билборды, растяжки)
...
- Покупка изданных книг для чтения и обучения (не печать): художественная
  литература (badiiy adabiyot), учебники (darslik), детская и научная литература
  для библиотек, школ, вузов, колоний; «Книги печатные» / «Bosma kitoblar» от
  библиотеки (axborot-kutubxona) или учреждения; «kitoblar olish / xarid
  qilish». Тетради — туда же. Правило только про книги.
  НО печать книги заказчика — НАШЕ: «kitobini chop etish», «nashr qilish»,
  тираж, макет, техзадание на печать, заказ издательства (nashriyot) на печать
  тиража, издательские и типографские услуги, переплёт. Книги и журналы
  учёта/регистрации — бланочная продукция, НАШЕ
...
(печать, упаковка, печать книг/изданий на заказ, бланки, бейджи, таблички,
стенды) — оцени
лот как НАШ (score >= 70), даже если остальные позиции не наши.
ИСКЛЮЧЕНИЕ — СНАБЖЕНЧЕСКИЙ ПЕРЕЧЕНЬ ГОТОВЫХ ТОВАРОВ. Оно срабатывает ТОЛЬКО
когда в лоте НЕСКОЛЬКО позиций и большинство из них — закупка готового для
нужд офиса или хозяйства (ручки, карандаши, маркеры, ластики, скобы, скрепки,
клей, линейки, калькуляторы, файлы, а также мыло, стиральный порошок, моющие
средства, посуда, лампочки, инструмент). В таком перечне попавшие туда же
папка, блокнот, тетрадь или бумажная закладка НЕ делают лот нашим: их берут со
склада как товар, заказа на ИЗГОТОВЛЕНИЕ здесь нет. Такой лот НЕ наш (score < 40).
  мыло + порошок + моющее + папка          = НЕ наш (снабжение)
  карандаши + ручки + скобы + блокнот      = НЕ наш (снабжение)
  «Картон конверт» отдельной позицией      = НАШ (заказ на изготовление)
  стикер + флаер + закладка + крафт-пакет  = НАШ (печатная продукция)
Исключение НЕ применяется к лоту из одной позиции и не применяется, если
названы изготавливаемые печатные изделия (стикеры, флаеры, этикетки, конверты,
крафт-пакеты, бланки, визитки, каталоги, брошюры) ЛИБО указаны тираж, макет,
логотип, нанесение, печать.
...
# салфетницы.
#
# Оговорка про бумагу дописана в тот же день после независимой проверки Данияра
# (два живых лота). Условие «нанесение должно быть в запросе» я вводил ТОЛЬКО
# для мерча из дерева/кожи/стекла, но модель распространила его на картон:
# «Картон конверт» (встречный аукцион Sanoat-Qurilish Bank на 234 850 000 сум)
# стал дропаться с причиной «готовые конверты без печати — закупка». Проверено
# сравнением с промптом ДО правки: он держал 10/10 keep, мой — 9/10 плюс 2 из 4
# дропов в replay, то есть я сам внёс нестабильность на живом лоте банка.
# Для бумаги и картона условие неверно по существу: keep-список прямо называет
# «готовые пакеты без логотипа» нашим товаром. После оговорки — 10/10 keep,
# 44/44 корпуса, на 104 живых лидах единственное расхождение оказалось
# плаванием самого прода (кандидат там как раз стабильнее).
#
# ЗАКРЫТО 04.08 решением Данияра: «закупка готовой канцелярии» — DROP. Правило
# записано не здесь, а в мульти-лотовую оговорку _RELEVANCE_PROMPT, потому что
# замер показал: эти лоты идут ТЕНДЕРНЫМ трактом, а не чатовым. Из семи живых
# перечней канцтоваров шесть вообще не доходили до AI (умирали на цене или
# дедлайне), а седьмой — 8164341, карандаши/ручки/ластики/скобы плюс блокнот —
# плавал: score 20, 0, 75 при трёх прогонах, то есть keep 1 из 3. Виновата была
# сама оговорка «хотя бы одна позиция наша»: блокнот в снабженческом перечне её
# запускал. Контроль, который правка НЕ должна сломать, — 8186383 «Стикер,
# Флаер, Закладка канцелярская бумажная, Крафт-пакет» (corpus c0019).
...
- МЕРЧ И КОРПОРАТИВНЫЕ ПОДАРКИ из дерева, кожи, стекла, металла — когда на
  ...
  ЭТО УСЛОВИЕ ТОЛЬКО ПРО НЕ-БУМАЖНЫЕ предметы. Бумага и картон — наш товар
  сами по себе: «картонные конверты», «готовые пакеты без логотипа»,
  «коробки», «бумажные стаканчики» — keep и БЕЗ всякого нанесения.
...
#
# `21-10-2025` (день-месяц-год через дефис) до 30.07 не разбирался ни одним
# шаблоном: _parse_deadline возвращал None, гейт «дедлайн истёк» считал лот
# бессрочным, и такие строки не могли просрочиться никогда. Цена ошибки,
# измеренная по всем 6 500 отправленным алертам: 102 алерта в этом формате, 99
# из них — по лотам, закрывшимся раньше дня сбора (все из «E-Birja завершённые
# сделки», единственного источника с таким форматом среди отправленных).
#
# Порядок значим: победитель — ПОСЛЕДНИЙ совпавший шаблон в списке, а не самая
# поздняя дата. Дефисный день-месяц стоит ДО ISO, чтобы в строке с двумя датами
# («с 21-10-2025 до 2026-08-04») выигрывал ISO. Внутрь ISO-строки дефисный
# шаблон не влезает: после второго дефиса там остаётся две цифры, а нужно четыре.
_DATE_PATTERNS = [
...
_NO_PUSH_SOURCES = {
    ...
    # Все е-магазины UZEX выключены из алертов 22.08 по решению Данияра
    # («не нужны алерты из е-магазина UZEX вообще») после того, как он прокликал
    # девять крупнейших алертов дороже 15 млн. Проверено в браузере: каждая
    # карточка `/home/shop/detail/` — это ОФЕРТА ПРОДАВЦА, а не запрос
    # покупателя. «Ежедневник» на 100 млн оказался 500 шт по 200 тыс. и одной
    # из 139 аналогичных оферт; «Бумага немелованная» на 180 млн — Double A из
    # Таиланда. Подать по ним нельзя в принципе.
    #
    # Цена решения: −427 алертов за 30 дней (185 + 162 + 80), это 32% потока,
    # примерно −14 в сутки. Первая редакция писала «−443», приплюсовав 16 у
    # соседних е-магазинов (Ebirja, Cooperation) — а они НЕ выключены.
    # Поймано независимой проверкой. Дайджест освобождается для
    # лотов, по которым можно подать.
    #
    # Сбор НЕ выключен: строки остаются в базе и на Vercel как ценовой
    # бенчмарк — 139 оферт на ежедневник это готовая рыночная вилка.
    "UZEX Э-магазин рекламные услуги",  # off-profile (ads placement, not our print)
    "UZEX Э-магазин печатные услуги",
...
def _is_high_signal(t: RawTender) -> bool:
    ...
    #
    # Условие на срок — `not t.deadline` (поле ПУСТОЕ), а не `dt is None`.
    # Разница не косметическая: `dt is None` значит «не удалось разобрать», и
    # под неё попадает лот с ЖИВЫМ, но нечитаемым сроком. Проверено запуском:
    # «15 января 2020» (формат uz-kor, русское название месяца, наш парсер его
    # не берёт) при score 90 и пустой цене давал push=True, хотя срок истёк
    # шесть лет назад — гейт просрочки на нём тоже молчит по той же причине.
    # Радиус был реальный: 54 из 81 включённого источника не имеют селектора
    # цены, а uz-kor и railway вдобавок отдают нечитаемую дату (см. A7 аудита).
    # Найдено red-team 04.08 в тот же день, что и правка.
    if (t.relevance_score or 0) >= 90 and t.price is None and not t.deadline:
        return True
...
def _build_digest_keyboard(tenders: List[RawTender], start_seq: int) -> dict:
    ...
    Зачем вообще. До 11.08 у дайджеста не было кнопок НИ ОДНОЙ. Замер за 30 дней
    (11.08): из 1473 алертов 861 ушёл пушем и 612 — дайджестом, то есть 42%
    показанного человеку нельзя было оценить в принципе. У источников
    «э-магазин»/«оферт» это все 100%: у «UZEX Э-магазин печатные услуги» за две
    недели 84 алерта и ни одного отдельного сообщения. Обучение при этом
    питалось только пуш-хвостом: кликов нет с 01.08, понедельничный прогон
    дистиллятора обработал ноль коррекций.
