...
_RELEVANCE_PROMPT = """Наша компания — ТИПОГРАФИЯ и УПАКОВОЧНОЕ производство. Мы делаем:
...
- Баннеры, постеры, плакаты
- Сувенирная продукция (ручки, флешки, ежедневники)
- Печать на футболках, флагах, лентах

Это ЗАПРОС КЛИЕНТА — кто-то ИЩЕТ наши услуги?
Название: {title}
Заказчик: {organization}

ВАЖНО — ответь NO если:
- Это реклама ЧУЖИХ услуг (кто-то предлагает свои услуги)
- Это вакансия / поиск работы
- Это оклейка авто, наружная реклама, монтаж вывесок (не наш профиль)
- Это дизайн без печати
- Непонятно что конкретно нужно

Ответь YES только если клиент ищет ИМЕННО то что мы производим.
/no_think"""
