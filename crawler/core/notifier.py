...
# 32/32 лида корпуса без изменений. Оставшиеся 3 — НЕ этот класс, а граница
# «изготовление мерча из дерева/кожи с лого» и «закупка готовой канцелярии»;
# решение за Данияром, поэтому не закодировано (см. main.md).
...
- ИЗГОТОВЛЕНИЕ изделия из не-бумажных материалов: силикон, резина, металл,
  пластик, оргстекло — «сделать силиконовый браслет», «резиновый логотип»,
  «объёмная буква». НЕ путать с ПЕЧАТЬЮ на готовом изделии (визитки на
  металле, лого на бокале) — это НАШЕ
