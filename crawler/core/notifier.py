...
Оцени объявление ниже — относится ли оно к нашему профилю? Это может быть тендер, запрос клиента ("кто делает коробки?"), или закупка. Думай как коммерческий директор: важна суть, а не совпадение слов. "пакет" может быть пакет документов, "печать" — канцелярская печать, "календарный" — про сроки.

Объявление:
Название: {title}
Заказчик: {organization}

Ответь YES если объявление по нашему профилю (полиграфия, упаковка, печать, этикетки, коробки), NO если нет.
/no_think"""
