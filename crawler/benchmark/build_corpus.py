...
def add_golden_leads(b, skip_texts=()):
    ...
    for i, (text, lab, why) in enumerate(GOLDEN_LEADS):
...
def append_curated(path):
    ...
    marker = ("v2 → v2.1 (29.07): ДОБАВЛЕНЫ записи класса «смешанная корзина» "
              "(наша позиция рядом с чужой + контрольные омонимы), метки "
              "существующих не тронуты — минорная версия, дельта внутри v2.x "
              "осмысленна.")
