...
Что проверяет — четыре оси, потому что падение объёма само по себе не говорит,
где оно родилось, а медленное сползание вообще не видно в сравнении с базой:
  ...
  D. СПОЛЗАНИЕ — форма недельной кривой. Проверки A и B сравнивают со базой,
               которая при долгом тренде съезжает вместе с ним, поэтому обе
               молчали бы 12.07 и 19.07 (см. блок про TREND_* ниже). Эта ось
               смотрит не на величину падения, а на то, что оно идёт третью
...
Крон ставится через ops/install-funnel-cron.sh (06:40 ежедневно).
...
MIN_SRC_ALERTS = 10      # источник считаем «дававшим», если в базе было столько.
#                          Было 5 — и на здоровой неделе 05.07 сторож звенел на
#                          источнике с 7 алертами: мелкие фиды замолкают сами по
#                          себе. Ось источников вспомогательная, ложная тревога
...
# Оба списка переехали в crawler/core/source_health.py (05.09): туда же смотрят
# freshness_watchdog, healthcheck, zero-result-трекер и реестр здоровья.
# Категории разные и сохранены: alerts_off — «алерты выключены осознанно»,
# tightened — «их стало меньше из-за гейта». Имена здесь прежние.
from crawler.core.source_health import (  # noqa: E402
    ALERTS_OFF_SOURCES as KNOWN_SILENT,
    TIGHTENED_SOURCES as KNOWN_TIGHTENED,
)
...
def trend_slide(buckets):
    ...
    """
    need = TREND_WEEKS + 1
    # Неполные корзины выбрасываем: на краю окна попадается огрызок недели
    # (в прогоне 30.07 — 9.1 алертов/день на двух днях), и он читается как
    # «шаг вверх», ломая или, наоборот, выдумывая серию.
    tail = [b for b in buckets if b["days"] >= 5][-need:]
    if len(tail) < need:
        return None
...
def verdict(rep):
    ...
    if d is not None and d >= DROP_PCT:
        ...
        # Тревогу НЕ гасим — регрессия обязана быть видна, — но рядом кладём уже
        # разобранные причины. Иначе читатель каждую неделю выводит их заново, а
        # на четвёртый раз перестаёт читать вовсе, и настоящая поломка тонет в
        # привычной цифре. Ровно это и случилось: сторож кричал с 14.07, причины
        # (01.07, 13.07, 30.07) лежали в коммитах, и три недели никто их не свёл.
        for src, why in sorted(KNOWN_TIGHTENED.items()):
            notes.append("объём: часть падения объяснена — «%s»: %s" % (src, why))
        if KNOWN_SILENT:
            notes.append("объём: плюс %d источников с объяснённым молчанием (см. выше)"
                         % len(KNOWN_SILENT))
