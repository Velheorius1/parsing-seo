...
class HealthCheck:
    ...
    def check_dead_sources(self):
        ...
        """
        # Список объяснённого молчания переехал в crawler/core/source_health.py
        # (05.09): его же читает zero-result трекер, чтобы недельная сводка не
        # требовала действия по источникам, решение о которых уже принято.
        from crawler.core.source_health import DEAD_SOURCES_WHITELIST
        try:
            client = self._get_client()
