...
ДВА РАЗНЫХ ВОПРОСА — НЕ СМЕШИВАЙ ИХ.
`profile` — про РАБОТУ: относится ли предмет закупки к нашему производству. Дедлайн,
статус лота, цена и конкуренты здесь НИ ПРИ ЧЁМ. Лот, который уже разыгран, всё равно
«наш профиль», если печатали журнал.
`verdict` — про ДЕЙСТВИЕ СЕЙЧАС: имеет ли смысл заходить. Здесь как раз и учитываются
срок, статус и конкуренция; по закрытому лоту вердикт «пропустить» даже при нашем профиле.
Смешение этих двух ответов — источник самых дорогих ошибок: лот верно опознан как наш,
но помечен «пропустить» по сроку, и в следующий раз похожий лот отбрасывается как чужой.
...
ДВЕ ГРАНИЦЫ, ПОДТВЕРЖДЁННЫЕ ВЛАДЕЛЬЦЕМ (10.08.2026, разбор по документам).

1. «Закупка ДЛЯ типографии» — это не «заказ типографии». Лот 501515 назывался
«Типография махсулотларини харид қилиш», а в ТЗ — картриджи Canon и Xerox, ролики
подачи бумаги Duplo, фольга и плёнка для переплёта. Расходники и запчасти для чужого
производства: НЕ наш профиль. Само слово «типография» в названии ничего не решает —
решает, ЧТО закупают: нашу работу или материалы, которыми работают другие.

2. Граница по пакетам проходит НЕ по слову «пакет», а по печати и материалу.
Крафт-пакеты С ПЕЧАТЬЮ — наш заказ (владелец подтверждал четырежды). Лот 502596
«Полиэтиленовые пакеты» по ТЗ оказался мешками HDPE/LDPE для сбора отходов,
900x1200 мм, 650 мкм, без печати — это экструзия плёнки, НЕ наш профиль.
То же и с упаковкой вообще: коробка, папка, конверт, картхолдер под печать — наши;
голая полимерная тара без печати — нет."""
...
TOOLS = [
    ...
    _fn("submit_verdict",
        ...
        params={
            ...
            "profile": {"type": "string", "enum": ["наш профиль", "не наш профиль", "неясно"],
                        "description": "Относится ли ПРЕДМЕТ лота к нашему производству. "
                                       "Только про работу — БЕЗ учёта дедлайна, статуса и цены: "
                                       "уже разыгранный лот на печать журнала всё равно «наш профиль»."},
...
def _format_verdict_msg(seq, v):
    ...
    # Профиль показываем ОТДЕЛЬНОЙ строкой: «пропустить» по сроку и «пропустить»
    # потому что не наша работа — разные новости, и по первой Данияр может успеть
    # поискать тот же предмет на другой площадке.
    prof = {"наш профиль": "✅ наш профиль", "не наш профиль": "⛔️ не наш профиль",
            "неясно": "❔ профиль неясен"}.get(v.get("profile"))
    parts = ["🔍 *Разбор лота #%s*" % seq,
             "%s *Вердикт: %s*%s" % (emoji, v.get("verdict", "?").upper(),
                                     ("   ·   " + prof) if prof else ""),
             v.get("why", "")]
